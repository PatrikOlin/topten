defmodule TopTenWeb.ListLive.Edit do
  ...
  def mount(%{"slug" => slug}, _session, socket) do
    case Lists.get_list_by_slug!(slug) do
      %TopTenList{} = list ->
        # Convert DB items to the format used in the form
        form_items = 
          Enum.sort_by(list.items, & &1.position)
          |> Enum.map(fn item -> 
            %{
        ...
        next_position = (List.last(form_items) || %{position: 0}).position + 1
          
        {:ok,
         socket
         |> assign(:list, list)
         |> assign(:changeset, Lists.change_top_ten_list(list))
         |> assign(:items, form_items)
      ...
      _ ->
        {:ok,
         socket
         |> put_flash(:error, "Lista hittades inte.")
         |> redirect(to: ~p"/")}
    end
  end
  ...
  def handle_event("save", %{"top_ten_list" => list_params, "items" => items_params}, socket) do
    ...
    case Lists.update_top_ten_list(socket.assigns.list, list_params, items) do
      {:ok, list} ->
        {:noreply,
         socket
         |> put_flash(:info, "Lista uppdaterad!")
  ...
  def render(assigns) do
    ...
    <div class="container mx-auto px-4 py-8">
      <h1 class="text-3xl font-bold mb-6">Redigera lista</h1>
      ...
      >
		...
		<div class="space-y-3">
          ...
          <div class="pt-4">
            <.button class="dark:bg-lorange-100 dark:text-stone-200 dark:hover:bg-lorange-80" type="submit" phx-disable-with="Uppdaterar...">
              Spara ändringar
            </.button>
            <a href={~p"/lists/#{@list.slug}"} class="ml-4 text-gray-600 dark:text-stone-200 hover:underline">Avbryt</a>
          </div>
        </div>
      </.form>
    </div>
